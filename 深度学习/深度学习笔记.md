...
近年来，随着Transformer、MOE架构的提出，使得深度学习模型轻松突破上万亿规模参数，从而导致模型变得越来越大，因此，我们需要一些**大模型压缩技术**来降低模型部署的成本，并提升模型的推理性能。而大模型压缩主要分为如下几类：
...
知识蒸馏的过程分为2个阶段: 原始模型训练（Net-T训练）、精简模型训练（Net-S训练）
即 **第一步**是训练Net-T；**第二步**是在高温T下，蒸馏Net-T的知识到Net-S

知识蒸馏时，由于我们已经有了一个泛化能力较强的Net-T，我们在利用Net-T来蒸馏训练Net-S时，可以直接**让Net-S去学习Net-T的泛化能力**。
所以把教师的softmax层的输出作为学生的soft target
...
![img](https://raw.githubusercontent.com/kakarotto007/final/master/v2-29a851c6fa9cc809e51ce738abbec2ce_720w.webp)
...
![img](https://raw.githubusercontent.com/kakarotto007/final/master/v2-a9e90626c5ac6f64a7e04c89f6ce3013_720w.webp)

### softmax函数

原始的softmax：

![image-20231122163616432](https://raw.githubusercontent.com/kakarotto007/final/master/image-20231122163616432.png)

但要是直接使用教师的softmax层的输出值作为soft target, 这又会带来一个问题: 当softmax输出的概率分布熵相对较小时，负标签的值都很接近0，对损失函数的贡献非常小，小到可以忽略不计。因此**"温度"**这个变量就派上了用场。

加了温度之后的softmax：

![image-20231122163705124](https://raw.githubusercontent.com/kakarotto007/final/master/image-20231122163705124.png)
...
![img](https://raw.githubusercontent.com/kakarotto007/final/master/v2-d01f5142d06aa27bc5e207831b5131d9_720w.webp)

总的loss包括 soft label 的loss和hard target的 loss，loss用的是多分类的交叉熵损失函数。

# 交叉熵损失函数

二分类问题：

![image-20231122170035051](https://raw.githubusercontent.com/kakarotto007/final/master/image-20231122170035051.png)

多分类问题：

![image-20231122170114021](https://raw.githubusercontent.com/kakarotto007/final/master/image-20231122170114021.png)
